...
const SAFE_ACTIONS = new Set(['access', 'session', 'signup', 'resend', 'verify', 'signin', 'forgot', 'reset', 'signout', ...LEAGUE_ACTIONS]);
...
function createHandler(clientFactory, env = {}) {
  return async function account(req, res) {
    ...
    try {
      ...
      if (!user || !user.email_confirmed_at) { clear(); return (LEAGUE_ACTIONS.has(action) || action === 'access') ? reply(401, {error:'Sign in to continue.'}) : reply(200, { user: null }); }
